...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('saved_filters', function (Blueprint $table) {
            $table->id();
            $table->string('name', 200); // نام فیلتر
            $table->text('description')->nullable(); // توضیحات
            $table->unsignedBigInteger('user_id'); // ایجادکننده
            $table->unsignedBigInteger('organization_id')->nullable(); // سازمان (اختیاری)
            $table->string('filter_type', 100); // نوع فیلتر (مثلاً family_search)
            $table->json('filters_config'); // تنظیمات فیلتر (JSON)
            ...
            $table->index(['filter_type', 'visibility', 'is_active']);
