<!-- منوی اصلی سایت -->
<div class="sidebar-wrapper relative transition-all duration-300">
    <div class="sidebar-menu" id="sidebar-menu">
        <div class="fixed h-screen right-0 top-0 w-64 bg-white shadow-md py-4 z-40 transition-all duration-300 border-l border-gray-100">
            <!-- Logo Section -->
            <div class="flex flex-col items-center justify-center py-5 border-b border-gray-200">
                <div class="transition-all duration-300 w-20 h-20">
                    <img src="{{ asset('images/image.png') }}" alt="لوگو خیریه" class="w-full">
                </div>
            </div>

            <!-- Menu Items -->
            <div class="flex-grow overflow-y-auto" style="max-height: calc(100vh - 140px);">
                <a href="{{ route('dashboard') }}" class="flex items-center py-3 px-6 hover:bg-gray-50 {{ request()->routeIs('dashboard') ? 'bg-gray-100' : '' }}">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-gray-700 ml-3 menu-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 5a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1H5a1 1 0 01-1-1V5zm10 0a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1h-4a1 1 0 01-1-1V5zM4 15a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1H5a1 1 0 01-1-1v-4zm10 0a1 1 0 011-1h4a1 1 0 011 1v4a1 1 0 01-1 1h-4a1 1 0 01-1-1v-4z" />
                    </svg>
                    <span class="menu-text font-medium">داشبورد</span>
                </a>

                <!-- منوی مخصوص خیریه -->
                @if(auth()->check() && (auth()->user()->user_type === 'charity' || auth()->user()->user_type === 'admin'))
                    <a href="{{ route('charity.insured-families') }}" class="flex items-center py-3 px-6 hover:bg-gray-50 transition {{ request()->routeIs('charity.insured-families') ? 'bg-green-500 text-white' : '' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 ml-3 menu-icon {{ request()->routeIs('charity.insured-families') ? 'text-white' : 'text-gray-700' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                        </svg>
                        <div class="flex flex-col menu-text">
                            <span class="font-medium">خانواده‌های بیمه شده</span>
                            
                            <span class="text-xs {{ request()->routeIs('charity.insured-families') ? 'text-white' : 'text-gray-500' }}">
                                @if(isset($insuredFamilies) && isset($insuredMembers))
                                    ({{ $insuredFamilies }} خانواده - {{ $insuredMembers }} نفر)
                                @else
                                    (۰ خانواده - ۰ نفر)
                                @endif
                            </span>
                        </div>
                    </a>
                    
                    <a href="{{ route('charity.uninsured-families') }}" class="flex items-center py-3 px-6 hover:bg-gray-50 transition {{ request()->routeIs('charity.uninsured-families') ? 'bg-red-500 text-white' : '' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 ml-3 menu-icon {{ request()->routeIs('charity.uninsured-families') ? 'text-white' : 'text-gray-700' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" />
                        </svg>
                        <div class="flex flex-col menu-text">
                            <span class="font-medium">خانواده‌های بدون پوشش</span>
                            
                            <span class="text-xs {{ request()->routeIs('charity.uninsured-families') ? 'text-white' : 'text-gray-500' }}">
                                @if(isset($uninsuredFamilies) && isset($uninsuredMembers))
                                    ({{ $uninsuredFamilies }} خانواده - {{ $uninsuredMembers }} نفر)
                                @else
                                    (۰ خانواده - ۰ نفر)
                                @endif
                            </span>
                        </div>
                    </a>

                    <a href="{{ route('charity.add-family') }}" class="flex items-center py-3 px-6 hover:bg-gray-50 transition {{ request()->routeIs('charity.add-family') ? 'bg-blue-500 text-white' : '' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 ml-3 menu-icon {{ request()->routeIs('charity.add-family') ? 'text-white' : 'text-green-500' }}" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
                        </svg>
                        <span class="menu-text font-medium">افزودن خانواده جدید</span>
                    </a>
                @endif

                <!-- منوی مخصوص بیمه -->
                @if(auth()->check() && (auth()->user()->user_type === 'insurance' || auth()->user()->user_type === 'admin'))
                    <a href="#" id="insurance-requests-menu-item" class="flex items-center py-3 px-6 hover:bg-gray-50 transition">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-gray-700 ml-3 menu-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                        </svg>
                        <span class="menu-text font-medium">درخواست‌های بیمه</span>
                    </a>
                @endif

                <!-- منوی فقط مخصوص ادمین -->
                @if(auth()->check() && auth()->user()->user_type === 'admin')
                    <a href="#" class="flex items-center py-3 px-6 hover:bg-gray-50 transition">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-gray-700 ml-3 menu-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                        </svg>
                        <span class="menu-text font-medium">مدیریت کاربران</span>
                    </a>

                    <a href="#" class="flex items-center py-3 px-6 hover:bg-gray-50 transition">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-gray-700 ml-3 menu-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
                        </svg>
                        <span class="menu-text font-medium">سازمان‌ها</span>
                    </a>
                @endif
            </div>
            
            <!-- Footer Menu Items -->
            <div class="border-t border-gray-200 py-2 mt-auto">
                @if(auth()->check() && auth()->user()->user_type === 'charity')
                    <a href="{{ route('charity.settings') }}" class="flex items-center py-3 px-6 hover:bg-gray-50 transition {{ request()->routeIs('charity.settings') ? 'bg-gray-100' : '' }}">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-gray-700 ml-3 menu-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                        </svg>
                        <span class="menu-text font-medium">تنظیمات</span>
                    </a>
                @endif

                <form method="POST" action="{{ route('logout') }}" class="w-full">
                    @csrf
                    <button type="submit" class="w-full flex items-center py-3 px-6 hover:bg-gray-50 transition text-red-500">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 ml-3 menu-icon" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                        </svg>
                        <span class="menu-text font-medium">خروج</span>
                    </button>
                </form>
            </div>
        </div>
    </div>

    <!-- دکمه‌ی باز/بسته کردن منو -->
    <button 
        id="sidebar-toggle-btn" 
        class="sidebar-toggle-btn fixed z-50 bg-green-500 hover:bg-green-600 text-white p-2 rounded-l-md shadow-md transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-green-400 focus:ring-opacity-50 right-64 top-1/2 -translate-y-1/2"
        aria-label="باز و بسته کردن منو"
        aria-expanded="true"
    >
        <svg id="collapse-icon" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
        </svg>
        <svg id="expand-icon" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 hidden" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            <path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7" />
        </svg>
    </button>
</div>

<style>
    /* استایل‌های اصلی منو */
    .sidebar-menu .fixed {
        transition: width 0.3s cubic-bezier(0.4, 0, 0.2, 1);
    }
    
    /* استایل‌های آیکون‌ها در حالت بسته منو */
    .sidebar-menu.collapsed .menu-icon {
        margin: 0 auto;
        display: flex;
        justify-content: center;
        width: 100%;
    }
    
    /* تنظیم آیتم‌های منو در حالت بسته */
    .sidebar-menu.collapsed a, 
    .sidebar-menu.collapsed button {
        padding-left: 0;
        padding-right: 0;
        justify-content: center;
    }
    
    /* مرکز کردن آیکون‌ها در حالت بسته منو */
    .sidebar-menu.collapsed .fixed.w-16 {
        display: flex;
        flex-direction: column;
        align-items: center;
    }
    
    /* انتقال نرم متن‌ها */
    .sidebar-menu .menu-text {
        display: inline-block;
        transition: opacity 0.2s ease, transform 0.2s ease;
        white-space: nowrap;
    }
    
    /* تنظیم دکمه باز/بسته کردن منو */
    .sidebar-toggle-btn {
        transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
    }
    
    /* در حالت بسته منو، جای دکمه هم باید تغییر کند */
    .sidebar-menu.collapsed ~ .sidebar-toggle-btn {
        right: 16px;
    }
    
    /* اضافه کردن انیمیشن چرخش برای آیکون */
    #collapse-icon, #expand-icon {
        transition: transform 0.3s ease;
    }
    
    /* اسکرول منو */
    .sidebar-menu .overflow-y-auto {
        scrollbar-width: thin;
        scrollbar-color: rgba(156, 163, 175, 0.5) transparent;
    }
    
    .sidebar-menu .overflow-y-auto::-webkit-scrollbar {
        width: 4px;
    }
    
    .sidebar-menu .overflow-y-auto::-webkit-scrollbar-track {
        background: transparent;
    }
    
    .sidebar-menu .overflow-y-auto::-webkit-scrollbar-thumb {
        background-color: rgba(156, 163, 175, 0.5);
        border-radius: 20px;
    }
</style>

<script>
    document.addEventListener('DOMContentLoaded', function() {
        const sidebarMenu = document.getElementById('sidebar-menu');
        const toggleBtn = document.getElementById('sidebar-toggle-btn');
        const collapseIcon = document.getElementById('collapse-icon');
        const expandIcon = document.getElementById('expand-icon');
        const menuTexts = document.querySelectorAll('.menu-text');
        
        // خواندن وضعیت قبلی منو از localStorage
        const isSidebarCollapsed = localStorage.getItem('sidebarCollapsed') === 'true';
        
        function collapseSidebar() {
            // تنظیم کلاس برای منو
            sidebarMenu.classList.add('collapsed');
            
            // تغییر عرض به 16px در حالت بسته
            const sidebarContainer = sidebarMenu.querySelector('.fixed');
            if (sidebarContainer.classList.contains('w-64')) {
                sidebarContainer.classList.remove('w-64');
                sidebarContainer.classList.add('w-16');
            }
            
            // مخفی کردن متن‌ها با انیمیشن
            menuTexts.forEach(el => {
                el.style.opacity = '0';
                el.style.transform = 'translateX(-10px)';
                setTimeout(() => {
                    el.style.display = 'none';
                }, 200);
            });
            
            // تغییر آیکون دکمه با انیمیشن
            collapseIcon.classList.add('hidden');
            expandIcon.classList.remove('hidden');
            
            // کوچک کردن لوگو
            const logoContainer = document.querySelector('.transition-all');
            if (logoContainer && logoContainer.classList.contains('w-20')) {
                logoContainer.classList.remove('w-20', 'h-20');
                logoContainer.classList.add('w-10', 'h-10');
            }
            
            // تغییر موقعیت دکمه toggle
            toggleBtn.style.right = '16px';
            toggleBtn.setAttribute('aria-expanded', 'false');
            
            // ذخیره وضعیت
            localStorage.setItem('sidebarCollapsed', 'true');
            
            // ارسال رویداد برای آگاه کردن سایر اسکریپت‌ها
            document.dispatchEvent(new CustomEvent('sidebar-toggle', { detail: { collapsed: true } }));
        }
        
        function expandSidebar() {
            // تنظیم کلاس برای منو
            sidebarMenu.classList.remove('collapsed');
            
            // تغییر عرض به 64px در حالت باز
            const sidebarContainer = sidebarMenu.querySelector('.fixed');
            if (sidebarContainer.classList.contains('w-16')) {
                sidebarContainer.classList.remove('w-16');
                sidebarContainer.classList.add('w-64');
            }
            
            // نمایش متن‌ها با انیمیشن
            menuTexts.forEach(el => {
                el.style.display = 'inline-block';
                // اجازه دهید DOM به‌روزرسانی شود قبل از انیمیشن
                setTimeout(() => {
                    el.style.opacity = '1';
                    el.style.transform = 'translateX(0)';
                }, 50);
            });
            
            // تغییر آیکون دکمه با انیمیشن
            expandIcon.classList.add('hidden');
            collapseIcon.classList.remove('hidden');
            
            // بزرگ کردن لوگو
            const logoContainer = document.querySelector('.transition-all');
            if (logoContainer && logoContainer.classList.contains('w-10')) {
                logoContainer.classList.remove('w-10', 'h-10');
                logoContainer.classList.add('w-20', 'h-20');
            }
            
            // تغییر موقعیت دکمه toggle
            toggleBtn.style.right = '64px';
            toggleBtn.setAttribute('aria-expanded', 'true');
            
            // ذخیره وضعیت
            localStorage.setItem('sidebarCollapsed', 'false');
            
            // ارسال رویداد برای آگاه کردن سایر اسکریپت‌ها
            document.dispatchEvent(new CustomEvent('sidebar-toggle', { detail: { collapsed: false } }));
        }
        
        // اعمال وضعیت ذخیره شده هنگام بارگذاری صفحه
        if (isSidebarCollapsed) {
            // اجرای با تأخیر برای اطمینان از اتمام بارگذاری DOM
            setTimeout(collapseSidebar, 50);
        }
        
        // افزودن رویداد کلیک به دکمه
        toggleBtn.addEventListener('click', function() {
            if (sidebarMenu.classList.contains('collapsed')) {
                expandSidebar();
            } else {
                collapseSidebar();
            }
        });
        
        // اضافه کردن قابلیت بستن منو با کلید ESC
        document.addEventListener('keydown', function(e) {
            if (e.key === 'Escape' && !sidebarMenu.classList.contains('collapsed')) {
                collapseSidebar();
            }
        });
    });
</script> 