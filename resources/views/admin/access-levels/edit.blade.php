    ...
    <div class="container mx-auto px-4 py-6">
        <div class="bg-white rounded-lg shadow-md p-6">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-lg font-semibold text-gray-700">ویرایش نقش: {{ $accessLevel->display_name ?: $accessLevel->name }}</h2>
            ...
            <form action="{{ route('admin.access-levels.update', $accessLevel) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" id="name" name="name" required
                               ...
                               value="{{ old('name', $accessLevel->name) }}">
                        @error('name')
                    ...
                    <div>
                        ...
                        <input type="text" id="display_name" name="display_name" required
                               class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                               value="{{ old('display_name', $accessLevel->display_name) }}">
                        @error('display_name')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                              placeholder="توضیح مختصری درباره این نقش...">{{ old('description', $accessLevel->description) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <div class="relative" x-data="{ 
                        open: false, 
                        selected: {{ old('parent_id', $accessLevel->parent_id) ?? 'null' }}, 
                        selectedText: '{{ old('parent_id', $accessLevel->parent_id) ? 
                            ($availableParents->where('id', old('parent_id', $accessLevel->parent_id))->first()->display_name ?? 
                             $availableParents->where('id', old('parent_id', $accessLevel->parent_id))->first()->name ?? 'نقش نامشخص') : 
                            'بدون والد (نقش اصلی)' }}' 
                    }">
                        <!-- Hidden Input -->
                        ...
                        <div x-show="open" @click.away="open = false"
                            ...
                            <button type="button" 
                                    @click="selected = {{ $parent->id }}; selectedText = '{{ $parent->display_name ?: $parent->name }}'; open = false"
                                    class="w-full px-3 py-2 text-right hover:bg-gray-50 flex items-center justify-between">
                                <span class="text-gray-900">{{ $parent->display_name ?: $parent->name }}</span>
                ...
                <div class="flex items-center">
                    <input type="checkbox" id="inherit_permissions" name="inherit_permissions" value="1"
                           ...
                           {{ old('inherit_permissions', $accessLevel->inherit_permissions) ? 'checked' : '' }}>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 max-h-64 overflow-y-auto border rounded-lg p-4">
                        ...
                        <div class="flex items-center">
                            <input type="checkbox" 
                                   ...
                                   class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                                   {{ $accessLevel->hasPermissionTo($permission->name) ? 'checked' : '' }}>
