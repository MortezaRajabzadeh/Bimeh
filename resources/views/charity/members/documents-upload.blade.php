<x-app-layout>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <div class="grid md:grid-cols-2 gap-8">
                    ...
                    <div class="bg-gray-50 rounded-lg p-6 border border-gray-200">
                            ...
                            <div class="bg-green-50 border border-green-200 rounded-md p-4 mb-4">
                                <div class="flex items-start">
                                    ...
                                    <div class="mr-3">
                                        ...
                                        <div class="mt-4">
                                            <a href="{{ $specialDiseaseDocument->getUrl() }}" target="_blank" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                        ...
                        <form action="{{ route('charity.family.members.documents.store', ['family' => $family->id, 'member' => $member->id]) }}" method="POST" enctype="multipart/form-data" class="mt-4">
                            @csrf
                    ...
                    <div class="bg-gray-50 rounded-lg p-6 border border-gray-200">
                        ...
                        @if($disabilityDocument)
                            ...
                            <div class="bg-green-50 border border-green-200 rounded-md p-4 mb-4">
                                <div class="flex items-start">
                                    ...
                                    <div class="mr-3">
                                        <h3 class="text-sm font-medium text-green-800">مدرک معلولیت آپلود شده است</h3>
                                        <div class="mt-2 text-sm text-green-700">
                                            <ul class="list-disc space-y-1 mr-5">
                                                <li>نام فایل: {{ $disabilityDocument->file_name }}</li>
                                                <li>تاریخ آپلود: {{ jdate($disabilityDocument->created_at)->format('Y/m/d H:i') }}</li>
                                                <li>حجم فایل: {{ round($disabilityDocument->size / 1024) }} کیلوبایت</li>
                                            </ul>
                                        </div>
                                        <div class="mt-4">
                                            <a href="{{ $disabilityDocument->getUrl() }}" target="_blank" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md shadow-sm text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 ml-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                                </svg>
                        ...
                        <form action="{{ route('charity.family.members.documents.store', ['family' => $family->id, 'member' => $member->id]) }}" method="POST" enctype="multipart/form-data" class="mt-4">
                            @csrf
                            <input type="hidden" name="document_type" value="disability">
                            ...
                            <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md hover:bg-gray-50 transition duration-150">
                                <div class="space-y-1 text-center">
                                    ...
                                    <div class="flex text-sm text-gray-600">
                                        <label for="disability_document" class="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none">
                                            <span>انتخاب فایل</span>
                                            <input id="disability_document" name="document" type="file" class="sr-only" accept=".jpg,.jpeg,.png,.pdf">
                                        </label>
                                        <p class="pr-1">یا فایل را اینجا رها کنید</p>
                                    </div>
                                    <p class="text-xs text-gray-500">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            ['special_disease_document', 'disability_document'].forEach(function(id) {
                ...
                fileInput.addEventListener('change', function() {
                    if (this.files.length) {
                        ...
                        fileInfo.className = 'mt-2 text-sm text-gray-900';
                        fileInfo.innerHTML = `فایل انتخاب شده: <span class="font-medium">${fileName}</span> (${fileSize} KB)`;
                        
                        // حذف اطلاعات فایل قبلی (اگر وجود دارد)
                        const existingInfo = dropZone.querySelector('.text-sm.text-gray-900');
                        if (existingInfo) {
                            existingInfo.remove();
                        }
