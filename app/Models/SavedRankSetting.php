...
class SavedRankSetting extends Model
{
    ...
    public function scopeVisible($query, $user)
    {
        return $query->where(function ($q) use ($user) {
            $q->where('user_id', $user->id) // تنظیمات خود کاربر
              ->orWhere('visibility', 'public') // تنظیمات عمومی
              ->orWhere(function ($subQuery) use ($user) {
                  $subQuery->where('visibility', 'organization')
                           ->where('organization_id', $user->organization_id); // تنظیمات سازمانی
              })
              ->orWhereHas('permissions', function ($permQuery) use ($user) {
                  $permQuery->where('user_id', $user->id)
                           ->where('permission_type', 'view'); // تنظیمات به اشتراک گذاشته شده
              });
    ...
    {
        return $query->where(function ($q) use ($user) {
            $q->where('user_id', $user->id) // مالک تنظیمات
              ->orWhereHas('permissions', function ($permQuery) use ($user) {
                  $permQuery->where('user_id', $user->id)
